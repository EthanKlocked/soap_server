...
export class FriendService {
    ...
    async areFriends(user1Id: string, user2Id: string): Promise<boolean> {
        try {
            ...
            throw new InternalServerErrorException('An unexpected error occurred while checking friendship');
        }
    }

    async sendFriendRequest(senderId: string, receiverId: string, message: string): Promise<FriendRequest> {
        try {
            if (!isValidObjectId(receiverId)) throw new BadRequestException('Invalid receiver ID format');
            if (await this.areFriends(senderId, receiverId)) throw new ConflictException('Users are already friends');
            const existingRequest = await this.friendRequestModel.findOne({
                senderId,
                receiverId,
                status: 'pending',
            }).exec();
            if (existingRequest) throw new ConflictException('A pending friend request already exists');
    ...
    async acceptFriendRequest(requestId: string): Promise<Friendship> {
        try {
            ...
            if (!request || request.status !== 'pending') throw new NotFoundException('Friend request not found or already processed');
            if (await this.areFriends(request.senderId.toString(), request.receiverId.toString())) throw new ConflictException('Users are already friends');
    ...
    async canSendFriendRequest(senderId: string, receiverId: string): Promise<boolean> {
        try {
            const lastRejectedRequest = await this.friendRequestModel.findOne({
                senderId,
                receiverId,
                status: 'rejected',
            }).sort({ lastRequestDate: -1 }).exec();
