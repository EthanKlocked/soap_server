...
export class DiaryService {
  ...
  constructor(
    ...
    const aiServiceHost = this.configService.get<string>(
      'AI_SERVICE_HOST_CUSTOM',
      'localhost',
    );
    const aiServicePort = this.configService.get<number>(
      'AI_SERVICE_PORT_CUSTOM',
      3000,
    );
  ...
  async create(userId: string, body: DiaryCreateDto): Promise<DiaryCreateDto> {
    try {
      ...
      this.analyzeAndSaveDiaryMetadata(
        userId,
        createdDiary._id,
        body.content,
      ).catch((error) => {
  ...
  async update(userId: string, id: string, body: DiaryUpdateDto) {
    try {
      //original chk
      const originalDiary = await this.diaryModel
        .findOne({ _id: id, userId: userId })
        .exec();
      if (!originalDiary) {
      ...
      if (body.content && body.content !== originalDiary.content) {
        this.analyzeAndSaveDiaryMetadata(userId, id, body.content).catch(
          (error) => {
