...
export class DiaryAnalysisService {
    ...
    async getSimilarUsers(userId: string, limit: number = 5, diaryId?: string): Promise<any[]> {
        try {
            ...
            const similarityScores = Object.entries(otherUserProfiles).map(([otherUserId, analyses]) => {
            ...
            throw new InternalServerErrorException('An unexpected error occurred while getting similar users');
        }
    }
    ...
    private createUserProfile(analyses: any[]): any {
        return {
            ...
            emotions: this.countOccurrences(analyses.flatMap(a => [a.primaryEmotion, a.secondaryEmotion].filter(Boolean))),
    ...
    private calculateJaccardSimilarity(set1: { [key: string]: number }, set2: { [key: string]: number }): number {
